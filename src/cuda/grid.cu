...
__host__ void GridData_FreeOnHost(GridData* grid_data) {
    if (grid_data) {
        if (grid_data->particle_hashes) {free(grid_data->particle_hashes);}
        if (grid_data->particle_indices) {free(grid_data->particle_indices);}
        if (grid_data->cell_starts) {free(grid_data->cell_starts);}
        if (grid_data->cell_ends) {free(grid_data->cell_ends);}
        free(grid_data);
    }
}
